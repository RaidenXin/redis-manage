 ...
 * @Modified By:
 */
public class RedisHashDataViewController implements DataViewController, Initializable {
    ...
    public void search() {
        ...
        if (StringUtils.isBlank(text)) {
            ...
            if (isFuzzySearch.isSelected()) {
                ScanResult<Pair<String, String>> data = hScan(client, START_INDEX, field);
                stack.clear();
                ...
                try {
                    ...
                    if (StringUtils.isNotBlank(value)) {
                        ...
                        nextIndex.set(START_INDEX);
                        items.clear();
                        items.add(new Pair<>(field, value));
                    } else {
                        stack.clear();
                        currentIndex.set(START_INDEX);
                        nextIndex.set(START_INDEX);
    ...
    private void setButtonEvent(RedisClient client, String current, String nextCursor){
        ...
        pre.setOnMouseClicked((event) -> {
            ...
            if (index != null){
                ...
                items.clear();
                //刷新数据
                ScanResult<Pair<String, String>> data = hScan(client, index, searchKey.getText());
                items.addAll(data.getResult());
                //将上一页标记为当前页
